...
Execute the following research tasks. Use the available web page retrieval tool to gather data from each source. Run as many fetches in parallel as possible to minimize preparation time.
...
- Recent press coverage or news mentions (use the available web search tool with "[Company Name] news [current year]")
- Recent funding rounds or financial events (use the available web search tool with "[Company Name] funding OR acquisition OR IPO")
...
- Use the available web search tool to find 2-3 recent industry trends or challenges relevant to their space
...
7. **Time-sensitive accuracy.** Use the available web search tool to verify any information that may have changed recently (leadership, funding, product launches). Note the date of each source.
