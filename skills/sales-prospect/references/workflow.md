...
You are the full prospect audit engine for `$sales prospect <url>`. You perform five analyses using permitted parallel workers or sequential execution, aggregate their results, and produce a unified PROSPECT-ANALYSIS.md report that is ready-to-use and deal-focused.
...
Use `the available web page retrieval tool` to retrieve the company homepage. Store the full content for subagent consumption.
...
```bash
python3 "$SALES_ROOT/scripts/analyze_prospect.py" --url <url> --output json
```
...
2. Perform manual extraction using the available web page retrieval tool data
...
## Phase 2: Five analyses

Use the host's available delegation tools only when parallel execution is permitted. Give each worker the discovery briefing, its role brief from `$SALES_ROOT/agents/`, and the corresponding skill. These Markdown role briefs are reference instructions, not registered agent types. If delegation is unavailable or disallowed, perform the five analyses sequentially in the current session.

Each analysis is independent. Workers return results to the coordinator and must not write shared reports. The coordinator performs synthesis after every available result is collected, then revises the outreach using the combined findings.
...
2. Mark the category unavailable. Do not invent a neutral score; report an incomplete score and the available weighted subtotal.
...
2. Mark the category unavailable and report evidence coverage
3. Add a note in the report naming the unavailable category; do not issue a complete composite score
...
2. Supplement with the available web search tool for external data (Crunchbase, LinkedIn, news)
