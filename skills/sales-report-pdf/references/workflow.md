...
### Step 5: Locate the PDF generation script

Resolve `SALES_ROOT` as described in the shared runtime instructions. The bundled script is `$SALES_ROOT/scripts/generate_pdf_report.py`. Verify it exists. If it is missing, report an incomplete installation and stop. Do not search the prospect folder or create a replacement script.
...
```bash
python3 "$SALES_ROOT/scripts/generate_pdf_report.py" _pdf_input.json "SALES-REPORT-$(date +%Y-%m-%d).pdf"
```
