...
Use `the available web page retrieval tool` to retrieve these pages (skip any already provided in the discovery briefing):
...
Use `the available web search tool` to find external data. Execute these searches:
...
| Public Presence | Speaking engagements, publications, podcasts | the available web search tool, conference sites |
